...
function setStaticEventListeners() {
    ...
    open_options.addEventListener("click", function () {chrome.runtime.openOptionsPage();}, false);
    ...
    // chrome.contextMenus.onClicked is only available in Service Worker in MV3
    try {
        chrome.contextMenus.onClicked.addListener(contextMenuClicked);
    } catch(e) {
        console.log('contextMenus API not available in popup (MV3):', e.message);
    }
}
...
function waitForBackgroundPage(callback) {
    ...
    try {
        ...
        // ignore cross-origin-like access issues, fallback to load listener
    }
...
function setPopupBackgroundState() {
    backgroundPage = getBackgroundPageProxy();
    if (!backgroundPage) {
        backgroundPage = window;
    }
    options = backgroundPage.options || {};
    stateInfo = backgroundPage.popupStateInfo || {};
    openFeedContainerClass = ((options.hideOthersOnOpen == true) ? "hidingFeedContainer" : "feedContainer");
    dataHeightAdjustment = options.hideOthersOnOpen ? 72 : 202;
}
...
var openFeeds = [];
var currentItem;
var openFeedContainerClass = "feedContainer";
var closedContainerClass = "feedContainer";
var openCount = 0;
var tabOpened = false;
var feedDataClass = "feedData";
var dataHeightAdjustment = 202;
var useGroups = false;
...
function main() {
    ...
    try {
        ...
        // MV3: onunload在popup关闭时不可靠，改用visibilitychange+pagehide
        var popupCloseHandler = function() {
            if (backgroundPage && backgroundPage.popupClosed) {
        ...
        window.addEventListener('pagehide', popupCloseHandler);
        document.body.onunload = popupCloseHandler;

        var styleSheets = document.styleSheets;
        if (backgroundPage && backgroundPage.applyStyleOptions) {
        ...
        if (!feedInfo) { feedInfo = {feeds: [], feedsByURL: {}}; }
    ...
    }); // close waitForBackgroundPage callback
    } catch(e) {
...
function createGroupDiv(group, pos) {
    ...
    groupText.addEventListener("click", function(e) { e.preventDefault(); });
...
function createFeedDiv(feed, pos, aboveDiv, container) {
    ...
    feedText.addEventListener("click", function(e) { e.preventDefault(); });
...
function createItemDiv(item, feedObject, i_idx, unseenDivs, divs, dataDiv) {
    ...
    // MV3: 阻止<a>标签默认导航行为，chrome://rssll只用于context menu识别
    titleText.addEventListener("click", function(e) { e.preventDefault(); });
...
function markFeedDivSeen(feed, div) {
    ...
    // 立即保存已读状态到localStorage
    if (backgroundPage && backgroundPage.updateSeenStates) {
        backgroundPage.updateSeenStates();
    }
}

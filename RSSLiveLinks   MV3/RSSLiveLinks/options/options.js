...
function waitForBackgroundPage(callback) {
    var frame = document.getElementById('backgroundFrame');
    if (!frame) {
        callback(window);
        return;
    ...
    function done() {
        ...
        called = true;
        frame.removeEventListener('load', done, false);
        callback(frame.contentWindow || window);
    }
    ...
    frame.addEventListener('load', done, false);
    setTimeout(done, 3000);
}

function load() {
    waitForBackgroundPage(function() {
        setOptionsBackgroundState();
...
function save_options() {
	...
	if (optionMods) {
		var bp = getBackgroundPageProxy();
		if (bp && bp.updateOptions) {
			bp.updateOptions(newOptions);
			options = bp.options || options;
		}
	}
...
function addNewGroup() {
	...
	if (newName == '') {
		...
		updateStatus(chrome.i18n.getMessage("opt_duplicateGroupName"), 3000);
	} else {
	...
	try {
		...
		if (file) {
			...
			reader.onload = function(evt) {
				...
				try {
					if (firstChar == '{') {
						...
						updateStatus("The import data does not appear to be JSON or XML", 4000);
					}
				} catch (err) {
					console.error(err);
				} finally {
					importCnfHide(true);
					if (ok) {
						// 关键修复：不再用 restore_options() 尝试"就地"把导入的新配置
						// 回填到已经打开的表单控件上。
						//
						// 之前的问题是：restore_options() 开头会先对每个样式分类无条件调用
						// setDefault()（把颜色选择器/字重/字体禁用并勾选"使用默认"），
						// 然后指望后面的 setStyleFields() 里 unsetDefault() 把它们正确恢复。
						// 这条链路涉及 jscolor 控件的内部状态、disabled 标记、"使用默认"复选框
						// 等好几层耦合，一旦哪个环节没对上（比如导入值恰好等于默认值、
		                // 或控件禁用状态未被及时清除），表单显示的就还是旧的默认状态。
						// 而只要用户之后点一次"保存"，getStyleFields() 就会读取这个
						// "看起来没变"的表单值，把刚导入的正确配置又覆盖回默认值。
						//
						// 直接刷新整个页面，让它按"正常打开页面"的路径重新初始化
						// （而不是"导入后临时打补丁"的特殊路径），可以彻底绕开这条
						// 脆弱的就地更新逻辑，从根源上避免这个问题。
						location.reload();
						return;
					}
				}
			...
			reader.onerror = function(e) {
				updateStatus("Import file read failed: " + e, 5000);
		...
		updateStatus("Import file read failed: " + err, 5000);
...
function importReplace(data) {
	...
	try {
		...
		updateStatus(chrome.i18n.getMessage("opt_badConfig") + ": " + err, 5000);
	} 
	return ok;
}
	...
	try {
		...
		if (txt.length > 0) {
			...
			if (docOK) {
				...
				updateStatus(chrome.i18n.getMessage("opt_badOPML"), 5000);
			}
		}
	} catch (err) {
		updateStatus(chrome.i18n.getMessage("opt_badOPML") + ": " + err, 5000);
	} 
	return ok;
}
...
function initExportHrefs() {
	// MV3: webkitRequestFileSystem已废弃，改用Blob+URL.createObjectURL下载
	initFileImport();

	configExport.addEventListener("click", function(e) {
		e.preventDefault();
		try {
			var data = backgroundPage.getConfigJSON();
			var blob = new Blob([data], {type: 'application/json'});
			var url = URL.createObjectURL(blob);
			var a = document.createElement('a');
			a.href = url;
			a.download = 'RSSLiveLinksConfig.json';
	...
	opmlExport.addEventListener("click", function(e) {
		e.preventDefault();
		try {
			var data = getOPMLText();
			var blob = new Blob([data], {type: 'application/xml'});
			var url = URL.createObjectURL(blob);
			var a = document.createElement('a');
			a.href = url;
			a.download = 'RSSLiveLinks.opml';
			document.body.appendChild(a);
			a.click();
			document.body.removeChild(a);
			setTimeout(function() { URL.revokeObjectURL(url); }, 1000);
...
function saveOPMLFile() {
	// MV3: 现在由 opmlExport click 事件处理导出
}

function saveConfigFile() {
	// MV3: 现在由 configExport click 事件处理导出
}

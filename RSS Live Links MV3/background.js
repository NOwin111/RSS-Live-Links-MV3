...
function checkServiceWorkerUpdates() {
	...
	chrome.storage.local.get(null, function(allData) {
		...
		for (var key in allData) {
			if (key.startsWith('swHasUpdate:')) {
				hasUpdates = true;
				keysToDelete.push(key);
			}
		}
		if (hasUpdates) {
			console.log("SW detected updates, triggering refresh");
			// 清除更新标记
			chrome.storage.local.remove(keysToDelete);
			// 强制刷新所有过期feed（清除expire让updateFeeds立即刷新）
	...
	try {
		if (feed && feed.data && feed.data.txt) {
			...
			if (feed.expire) {
				...
				// 同时保存到chrome.storage.local供service worker读取
				if (chrome.storage && chrome.storage.local) {
					var expireData = {};
					expireData["swExpire:" + feed.url] = {
						expire: feed.expire.getTime(),
						refreshTime: feed.refreshTime,
						name: feed.name
					};
					chrome.storage.local.set(expireData);
				}
			}
		}
	} catch(e) {
...
function stopAnimate() {
  ...
  chrome.action.setIcon({path:gfx.src});

...
async function updateBadge() {
    try {
        ...
        // SW完全没有seen数据时不更新角标
        if (Object.keys(seenStates).length === 0) {
            return;
        }
        ...
        for (const key in allData) {
            ...
            if (!feedData || !feedData.itemGuids || feedData.itemGuids.length === 0) continue;
            const url = key.substring('swFeed:'.length);

            // 该 feed 在 seenStates 中没有记录，跳过避免误计
            if (!(url in seenStates)) continue;

            const feedSeen = seenStates[url] || {};
            const hasUnseen = feedData.itemGuids.some(guid => {
    ...
    try {
        ...
        for (const feedInfo of expiredFeeds) {
            try {
                ...
                const existingFeedData = allData[feedKey] || {
                    updateCycleId: 0,
                ...
                let refreshTime = parseInt(feedInfo.refreshTime) || 60;
                if (feedInfo.refreshTime === 'TTL' && result.feed.ttl > 0) {
                    refreshTime = result.feed.ttl;
                }
        ...
        // Don't update badge here - popup saveSeenStates will trigger updateBadge
        // to avoid counting new guids that haven't been synced from popup yet

...
function saveSeenStates() {
	...
	// sync to SW for badge count via chrome.storage.local (avoids message size limits)
	try {
		chrome.storage.local.set({ swSeenStates: seenStates }, function() {
			chrome.runtime.sendMessage({greeting: "badgeSeenStatesSynced"});
		});
	} catch(e) { console.error("saveSeenStates sync failed: " + e); }
}

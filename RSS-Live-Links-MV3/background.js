...
function checkServiceWorkerUpdates() {
	...
	chrome.storage.local.get(null, function(allData) {
		...
		// 检查SW是否有比localStorage更新的feed数据
		feedInfo.feeds.forEach(function(feed) {
			var swFeedKey = 'swFeed:' + feed.url;
			var swFeedData = allData[swFeedKey];
			if (!swFeedData || !swFeedData.itemGuids) return;

			var localTxt = localStorage["feed:" + feed.url];
			// 如果SW的txt和localStorage的不同，说明SW后台更新了
			if (swFeedData.txt && swFeedData.txt !== localTxt) {
				console.log("SW has newer data for " + feed.name + ", restoring");
				// 用SW的数据更新localStorage
				try {
					saveToLocalStorage("feed:" + feed.url, swFeedData.txt);
					if (swFeedData.expire) {
						saveToLocalStorage("feedExpire:" + feed.url, swFeedData.expire.toString());
					}
				} catch(e) {}
				// 用SW数据恢复feed
		...
		if (hasUpdates) {
			console.log("SW updates applied, refreshing badge");
			updateSeenStates();
			// 通知SW更新角标
			try {
...
function restoreFeedFromCache(feed, txt) {
	...
	try {
		...
		if (!doc || doc.querySelector("parsererror")) {
			console.warn("Cached XML parse error for " + feed.name);
			return;
		}
		var data = {
			feed: feed.data,
...
function saveFeedText(feed) {
	try {
		if (feed && feed.data && feed.data.txt) {
			...
			try {
				var expireInfo = {
					expire: feed.expire ? feed.expire.getTime() : (Date.now() + 60 * 60 * 1000),
...
function setButtonTitle(upds, errors) {
	...
	// MV3: 如果popup的计数为0但SW有更大的计数，不清空角标让SW接管
	if (badgeText === '' || badgeText === '!') {
		// 延迟清空，让SW的updateBadge有机会重新设置
		setTimeout(function() {
